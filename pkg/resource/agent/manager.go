...
// +kubebuilder:rbac:groups=bedrockagent.services.k8s.aws,resources=agents,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=bedrockagent.services.k8s.aws,resources=agents/status,verbs=get;update;patch

var lateInitializeFieldNames = []string{"PromptOverrideConfiguration"}
...
func (rm *resourceManager) lateInitializeFromReadOneOutput(
	...
	if observedKo.Spec.PromptOverrideConfiguration != nil && latestKo.Spec.PromptOverrideConfiguration == nil {
		latestKo.Spec.PromptOverrideConfiguration = observedKo.Spec.PromptOverrideConfiguration
	}
...
func (rm *resourceManager) IsSynced(ctx context.Context, res acktypes.AWSResource) (bool, error) {
	...
	if r.ko.Status.AgentStatus == nil {
		return false, nil
	}
	agentStatusCandidates := []string{"PREPARED"}
	if !ackutil.InStrings(*r.ko.Status.AgentStatus, agentStatusCandidates) {
		return false, nil
	}
